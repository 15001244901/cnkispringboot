...
import net.cnki.cnkispringboot.Utilitys.*;
...
public class ExportController {
    ...
    public static void Export() {
        try {
            ...
            try {
                ips=Util.readFile("F://fabu/ips.txt");
            }catch (IOException e){
                e.printStackTrace();
            ...
            int excelModel=iparr[0].equals("1")?1:0;//1 六项 04项
            ...
            for (String ip :
                ...
                HashMap<String, String> maptab = new HashMap<>();//以查过的表或视图
                for (int index = 0; index < page; index++) {/**/
                    ...
                    for (ResultModel item :
                        ...
                        if (!StringUtils.isEmpty(item.getEXP())&&excelModel==1) {
    ...
    private static void exportExcel(HashMap<String, LinkedList<ExcelResultModel>> map,int type) {
        try {
            OutputStream out = new FileOutputStream("D:\\并行表统计" + DateUtil.getCurrentTimestamp() +(type==1?"[六项]":"[四项]") +".xls");
            String headerStr=type==1?"视图名,视图SQL,是否有效,无条件检索,数据表不存在,存在但无数据":"视图名,视图SQL,是否有效,无条件检索";
            ...
            for (String key : map.keySet()) {
                ...
                for (ExcelResultModel info :
                    ...
                    if (!StringUtils.isEmpty(info.getREMARK())&&type==1) {
                        for (String tab:
                                info.getREMARK().split(";")) {
                            setnotab.add(tab);
                        }
                    }
                    if (!StringUtils.isEmpty(info.getNODATA())&&type==1) {
                        for (String tab:
                ...
                if(setnotab!=null&&setnotab.size()>0&&type==1){
                    rowData3.add("其中数据表不存在的为： " + String.join(", ", setnotab) + "");
                    data.add(rowData3);
                }
                List rowData4 = new ArrayList();
                if(setnodata!=null&&setnodata.size()>0&&type==1){
